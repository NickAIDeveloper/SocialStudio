  ...
  if (loading) {
    return (
      <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-12 space-y-6">
        ...
        <p className="text-zinc-400 text-center">Connecting to Buffer...</p>
      ...
      <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-8">
        <div className="max-w-md mx-auto text-center space-y-5">
          ...
          <div>
            ...
            {error.detail && (
              <p className="text-xs text-zinc-500 mt-1 font-mono">{error.detail}</p>
            )}
          </div>

          <div className="bg-zinc-800/50 rounded-lg p-4 text-left">
            <p className="text-xs font-medium uppercase tracking-wider text-zinc-500 mb-3">Steps to fix</p>
            <ol className="space-y-3 text-sm text-zinc-300">
              ...
              <li className="flex items-start gap-3">
                ...
                <span className="pt-0.5">Add it to <code className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded font-mono text-zinc-300">.env.local</code> as <code className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded font-mono text-teal-400">BUFFER_API_KEY</code></span>
              </li>
              <li className="flex items-start gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-teal-500/15 border border-teal-500/30 flex items-center justify-center text-xs font-semibold text-teal-400">4</span>
                <span className="pt-0.5">Restart the dev server</span>
              </li>
            </ol>
          </div>
  ...
  if (allChannels.length === 0) {
    return (
      <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800/50 rounded-xl p-12 text-center space-y-4">
        ...
        <p className="text-zinc-400">No channels found in Buffer.</p>
        <p className="text-zinc-500 text-sm">Connect your Instagram accounts in Buffer, then refresh.</p>
        <button
          onClick={loadChannels}
          className="inline-flex items-center px-5 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 hover:text-white transition-colors active:scale-[0.98]"
        >
          Refresh
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-zinc-400">{allChannels.length} connected channel{allChannels.length !== 1 ? 's' : ''}</p>
        <button
          onClick={loadChannels}
          className="inline-flex items-center px-4 py-2 rounded-lg bg-zinc-800 border border-zinc-700 text-sm text-zinc-300 hover:bg-zinc-700 hover:text-white transition-all duration-200 active:scale-[0.98] cursor-pointer"
        >
          Refresh
        </button>
      </div>
            ...
            <div key={channel.id} className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800/50 rounded-xl overflow-hidden transition-all duration-200 hover:border-zinc-700/70">
              <div className="p-4 flex items-center gap-3">
                {channel.avatar ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-500 text-lg font-bold">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary" className="bg-zinc-800 text-zinc-400 text-xs capitalize">{channel.service}</Badge>
